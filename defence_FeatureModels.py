...
Additions (non-invasive):
  - Combined artifacts in results/_combined:
      * combined_metrics.json / combined_metrics.csv (per-model-per-attack)
      * aggregate_summary.json (means/stdevs by attack)
      * grouped bar charts: detector AUROC/AP, acceptance rates (overall/NAT/ADV),
        base AUC on ADV slice and accepted ADV.
      * README_COMBINED.txt (what’s inside)
  - Per-model stepwise sample trace (~5 total) saved as
      results/<Model>/mixed_<FGSM|PGD>/sample_trace.json
    showing: indices, label, z-before, (for ADV) z-after-attack, δ norms, defense p_adv pre/post,
    τ decision, base prob before/after, and prediction flips.
"""
...
CFG = {
    ...
    # sample trace
    "trace_k_total": 5,     # ~5 samples across NAT+ADV
    "trace_max_features_dump": 128,  # truncate z vectors if very high-D
}

LABELS = [0, 1]

# ---------- NEW: in-memory collector for combined metrics ----------
_COMBINED_ROWS = []  # appended inside mixed_eval
...
# ---------- NEW: small helpers for combined outputs ----------
def _grouped_bar(ax, df, metric, title, ylabel):
    # df columns: base_model, attack, <metric>
    models = sorted(df["base_model"].unique().tolist())
    attacks = sorted(df["attack"].unique().tolist())  # e.g., ["FGSM","PGD"]
    x = np.arange(len(models), dtype=float)
    width = 0.35 if len(attacks) == 2 else 0.6
    for i, atk in enumerate(attacks):
        sub = df[df["attack"] == atk]
        vals = [float(sub[sub["base_model"] == m][metric].values[0]) if not sub[sub["base_model"] == m].empty else np.nan for m in models]
        ax.bar(x + (i - (len(attacks)-1)/2)*width, vals, width, label=atk)
...
def _write_combined_artifacts(results_root: Path):
    if not _COMBINED_ROWS:
        return
    cdir = ensure_dir(results_root / "_combined")

    # Tabular combined metrics
    df = pd.DataFrame(_COMBINED_ROWS)
    df.to_csv(cdir / "combined_metrics.csv", index=False)
    save_json(json.loads(df.to_json(orient="records")), cdir / "combined_metrics.json")

    # Aggregates by attack
    agg = {}
    for atk in sorted(df["attack"].unique().tolist()):
        part = df[df["attack"] == atk]
        metrics = [
            "detector_auroc", "detector_ap",
            "accept_rate_overall", "accept_rate_nat", "accept_rate_adv",
            "base_auc_nat_slice", "base_auc_adv_slice",
            "base_auc_accepted_adv", "base_auc_accepted_nat"
        ]
        agg[atk] = {}
        for m in metrics:
            if m in part.columns:
                mu, sd = _nanmeanstd(part[m].values)
                agg[atk][m] = {"mean": mu, "std": sd}
    save_json({"generated_at": datetime.now(timezone.utc).isoformat(), "by_attack": agg}, cdir / "aggregate_summary.json")

    # Plots: single-figure grouped bars per metric (no collages)
    def _plot_metric(metric, title, ylabel, fname):
        if metric not in df.columns: return
        fig, ax = plt.subplots(figsize=(9.5, 5.5))
        _grouped_bar(ax, df, metric, title, ylabel)
        fig.tight_layout(); fig.savefig(cdir / fname, dpi=200); plt.close(fig)

    _plot_metric("detector_auroc", "Detector AUROC (NAT vs ADV) by Model", "AUROC", "detector_auroc_by_model.png")
    _plot_metric("detector_ap", "Detector Average Precision (ADV as +) by Model", "AP (PR-AUC)", "detector_ap_by_model.png")
    _plot_metric("accept_rate_overall", "Overall Acceptance Rate by Model", "acceptance", "accept_rate_overall_by_model.png")
    _plot_metric("accept_rate_nat", "NAT Acceptance Rate by Model", "acceptance", "accept_rate_nat_by_model.png")
    _plot_metric("accept_rate_adv", "ADV Acceptance Rate by Model", "acceptance", "accept_rate_adv_by_model.png")
    _plot_metric("base_auc_adv_slice", "Base AUROC on ADV Slice (pre-defense) by Model", "AUROC", "base_auc_adv_slice_by_model.png")
    _plot_metric("base_auc_accepted_adv", "Base AUROC on ACCEPTED ADV (post-defense) by Model", "AUROC", "base_auc_accepted_adv_by_model.png")

    # README (short, for your paper appendix)
    readme = f"""Combined artifacts directory

Files:
- combined_metrics.csv/json : per (model × attack) tidy rows of key metrics.
- aggregate_summary.json     : mean ± std by attack across models.
- detector_auroc_by_model.png, detector_ap_by_model.png
- accept_rate_*_by_model.png : overall/NAT/ADV acceptance rates.
- base_auc_adv_slice_by_model.png, base_auc_accepted_adv_by_model.png

Generated: {datetime.now(timezone.utc).isoformat()}
"""
    save_text(readme, cdir / "README_COMBINED.txt")
...
# ---------- NEW: sample trace utilities ----------
def _truncate_vec(vec: np.ndarray, max_len: int):
    v = np.asarray(vec, dtype=float).tolist()
    if len(v) <= max_len:
        return {"values": v, "truncated": False, "total_dim": len(v)}
    return {"values": v[:max_len], "truncated": True, "total_dim": len(v)}

def _score_base_small(base_name: str, base_est, Xz_small: np.ndarray, device, use_amp=False):
    if Xz_small.size == 0: return np.array([], dtype=np.float32)
    if base_name in {"DL-MLP","DL-FTTransformer"}:
        return eval_torch_batched(base_est, Xz_small, device, batch_size=max(8, min(128, Xz_small.shape[0])),
                                  desc=f"{base_name}/trace eval", use_progress=False, use_amp=use_amp)
    else:
        scores = score_vector(base_est, Xz_small)
        return sigmoidify(scores).astype(np.float32, copy=False)

def _defense_p_adv_small(defense, scaler, Xz_small: np.ndarray, calibrator, fast_predict, early_stop_margin):
    if Xz_small.size == 0: return np.array([], dtype=np.float32)
    xb_raw = scaler.inverse_transform(Xz_small).astype(np.float32, copy=False)
    return defense_p_adv(defense, xb_raw, batch=max(8, min(2048, Xz_small.shape[0])),
                         calibrator=calibrator, desc="defense/trace", use_progress=False,
                         fast_predict=fast_predict, early_stop_margin=early_stop_margin).astype(np.float32, copy=False)
...
def main():
    ...
    def distill_from_global(global_surr, teacher, Xz, device, epochs=CFG["surrogate_epochs_finetune"], batch=CFG["surrogate_batch"], hidden=CFG["surrogate_hidden"], sample_limit=CFG["surrogate_sample_limit"]):
    ...
    def craft_adv_subset(base_name: str, base_est, which: str, Xz_sub: np.ndarray, y_sub: np.ndarray) -> np.ndarray:
        ...
        surr = distill_from_global(global_surr, base_est, Xz, device=device)
        return attack_tabular_torch_batched(
            surr, Xz_sub, y_sub,
            eps=CFG["eps_z"], alpha=CFG["alpha_z"], steps=steps,
    ...
    def mixed_eval(base_name: str, base_est, which: str, adv_idx: np.ndarray, nat_idx: np.ndarray):
        ...
        if base_name in {"DL-MLP","DL-FTTransformer"}:
            base_probs_nat = eval_torch_batched(
                base_est, Xz[nat_idx], device, CFG["torch_eval_bs"],
                desc=f"{base_name}/eval NAT slice", use_progress=CFG["use_progress"], use_amp=CFG["use_amp"]
            )
            base_probs_adv = eval_torch_batched(
                base_est, Xz_adv, device, CFG["torch_eval_bs"],
                desc=f"{base_name}/eval ADV slice", use_progress=CFG["use_progress"], use_amp=CFG["use_amp"]
            )
        else:
            base_scores_nat = chunked_scores(
                base_est, Xz[nat_idx], CFG["eval_bs"],
                desc=f"{base_name}/score NAT slice", use_progress=CFG["use_progress"]
            )
            base_probs_nat = sigmoidify(base_scores_nat)
            base_scores_adv = chunked_scores(
                base_est, Xz_adv, CFG["eval_bs"],
                desc=f"{base_name}/score ADV slice", use_progress=CFG["use_progress"]
            )
        ...
        y_is_adv = np.concatenate([
            np.zeros_like(p_adv_nat, dtype=int),
        ...
        # --------- NEW: Append to combined rows (for results/_combined) ----------
        _COMBINED_ROWS.append({
            "base_model": base_name,
            "attack": which,
            "detector_auroc": float(det_auc),
            "detector_ap": float(det_ap),
            "accept_rate_overall": float(acc_rate_overall),
            "accept_rate_nat": float(acc_rate_nat),
            "accept_rate_adv": float(acc_rate_adv),
            "base_auc_nat_slice": float(auc_nat_slice),
        ...
        # --------- NEW: Sample trace (~5 across NAT+ADV) ----------
        try:
            trace_total = max(0, int(CFG.get("trace_k_total", 5)))
            if trace_total > 0:
                # split NAT/ADV roughly in half
                t_nat = min(nat_idx.size, max(0, trace_total // 2))
                t_adv = min(adv_idx.size, max(0, trace_total - t_nat))

                nat_positions = rng.choice(nat_idx.size, size=t_nat, replace=False) if t_nat > 0 else np.array([], dtype=int)
                adv_positions = rng.choice(adv_idx.size, size=t_adv, replace=False) if t_adv > 0 else np.array([], dtype=int)

                # NAT samples
                nat_global_idx = nat_idx[nat_positions] if t_nat > 0 else np.array([], dtype=int)
                z_nat = Xz[nat_global_idx] if t_nat > 0 else np.empty((0, Xz.shape[1]), dtype=np.float32)
                y_nat_true = y_base[nat_global_idx] if t_nat > 0 else np.array([], dtype=int)
                p_nat_base = base_probs_nat[nat_positions] if t_nat > 0 else np.array([], dtype=float)
                p_nat_def = p_adv_nat_full[nat_global_idx] if t_nat > 0 else np.array([], dtype=float)
                accept_nat_post = (p_nat_def < tau) if t_nat > 0 else np.array([], dtype=bool)

                nat_samples = []
                for k in range(t_nat):
                    bp = float(p_nat_base[k])
                    pred_pre = int(bp >= 0.5)
                    accepted = bool(accept_nat_post[k])
                    nat_samples.append({
                        "global_index": int(nat_global_idx[k]),
                        "true_label": int(y_nat_true[k]),
                        "z_before": _truncate_vec(z_nat[k], CFG["trace_max_features_dump"]),
                        "defense": {
                            "p_adv": float(p_nat_def[k]),
                            "tau": float(tau),
                        ...
                        "base": {
                            "prob_pre": bp,
                            "pred_pre": pred_pre,
                            "prob_post": bp if accepted else None,
                            "pred_post": pred_pre if accepted else None,
                            "flipped": False  # no pre/post change on NAT path
                        }
                    })

                # ADV samples (use crafted Xz_adv aligned to adv_idx)
                adv_global_idx = adv_idx[adv_positions] if t_adv > 0 else np.array([], dtype=int)
                z_adv_pre = Xz[adv_global_idx] if t_adv > 0 else np.empty((0, Xz.shape[1]), dtype=np.float32)
                z_adv_post = Xz_adv[adv_positions] if t_adv > 0 else np.empty((0, Xz.shape[1]), dtype=np.float32)
                y_adv_true = y_base[adv_global_idx] if t_adv > 0 else np.array([], dtype=int)
                p_adv_post_base = base_probs_adv[adv_positions] if t_adv > 0 else np.array([], dtype=float)
                p_adv_pre_base = _score_base_small(base_name, base_est, z_adv_pre, device, use_amp=CFG["use_amp"]) if t_adv > 0 else np.array([], dtype=float)
                p_adv_pre_def = _defense_p_adv_small(defense, scaler, z_adv_pre, calibrator, CFG["FAST_PREDICT"], CFG["EARLY_STOP_MARGIN"]) if t_adv > 0 else np.array([], dtype=float)
                p_adv_post_def = p_adv_adv[adv_positions] if t_adv > 0 else np.array([], dtype=float)
                accept_adv_post = (p_adv_post_def < tau) if t_adv > 0 else np.array([], dtype=bool)

                adv_samples = []
                for k in range(t_adv):
                    pre = float(p_adv_pre_base[k])
                    post = float(p_adv_post_base[k])
                    pred_pre = int(pre >= 0.5)
                    pred_post = int(post >= 0.5)
                    accepted = bool(accept_adv_post[k])
                    delta = z_adv_post[k] - z_adv_pre[k]
                    adv_samples.append({
                        "global_index": int(adv_global_idx[k]),
                        "true_label": int(y_adv_true[k]),
                        "z_before": _truncate_vec(z_adv_pre[k], CFG["trace_max_features_dump"]),
                        "z_after_attack": _truncate_vec(z_adv_post[k], CFG["trace_max_features_dump"]),
                        "delta_norms": {
                        ...
                        "defense": {
                            "p_adv_pre": float(p_adv_pre_def[k]),
                            "p_adv_post": float(p_adv_post_def[k]),
                            "tau": float(tau),
                            "decision_post": "accept" if accepted else "reject"
                        },
                        "base": {
                            "prob_pre": pre,
                            "pred_pre": pred_pre,
                            "prob_post": post if accepted else None,
                            "pred_post": pred_post if accepted else None,
                            "flipped": bool(pred_pre != pred_post)
                        }
                    })
                ...
                trace = {
                    "meta": {
                        ...
                        "dims": int(Xz.shape[1]),
                        "notes": "Sample-level trace of NAT/ADV through attack→defense→base pipeline."
                    },
                    "nat_samples": nat_samples,
                    "adv_samples": adv_samples
                }
                save_json(trace, out_dir / "sample_trace.json")
        except Exception as e:
            # Trace is optional; never break evaluation
            save_text(f"Trace generation failed: {repr(e)}", out_dir / "trace_error.txt")

        # Visuals for this scenario
        if CFG["use_plots"]:
    ...
    # ---------- NEW: write combined artifacts ----------
    _write_combined_artifacts(results_root)

    print("\n[DONE] Mixed-stream Defense-LGBM evaluation complete. Combined outputs at '<results>/_combined'.")
