...
Output directories and filenames remain UNCHANGED.
Only the *metrics JSON contents* now follow the richer schema used by Script 2:
  - keys: accuracy, precision, recall, f1, balanced_accuracy, mcc, kappa,
          roc_auc, pr_auc, specificity, confusion, brier, ece, log_loss_ce,
          samples, split, timestamp
  - thresholding: 0.5 for probabilities; 0.0 for margins (decision_function)

All other artifacts (candidate_summary.json, params_used.json, plots, calibrator, etc.)
...
def _sigmoidify(scores: np.ndarray) -> np.ndarray:
    ...
    return scores if (0.0 <= np.nanmin(scores) <= np.nanmax(scores) <= 1.0) else 1.0 / (1.0 + np.exp(-scores))

def _is_probability_vector(scores: np.ndarray) -> bool:
    if scores is None: return False
    smin, smax = float(np.nanmin(scores)), float(np.nanmax(scores))
    return (0.0 <= smin) and (smax <= 1.0)
...
# ---- Rich metrics pack (aligned to Script 2) ----
def rich_metrics(y_true, used_score, y_pred, proba=None, ece_bins=15):
    y_true = np.asarray(y_true).astype(int)
    ...
    # AUCs use the *score* (probability or margin if available, else y_pred fallback)
    try: out["roc_auc"] = float(roc_auc_score(y_true, used_score))
    except Exception: out["roc_auc"] = None
    try: out["pr_auc"] = float(average_precision_score(y_true, used_score))
...
def write_reports(result_dir: Path, tag: str, y, scores, ece_bins: int = 15):
    """
    Writes metrics_{tag}.json, classification_report_{tag}.txt,
    confusion_matrix_{tag}.png, roc_curve_{tag}.png

    NOTE: Only the *JSON schema* changes; filenames & plots unchanged.
    """
    # Decide how to threshold & what to report (probability vs margin vs labels)
    scores = np.asarray(scores).astype(float)
    is_proba = _is_probability_vector(scores)
    if is_proba:
        y_pred = (scores >= 0.5).astype(int)
        used_score = scores
        proba = scores
    else:
        # margin or hard labels fallback
        y_pred = (scores >= 0.0).astype(int) if np.unique(scores).size > 2 else scores.astype(int)
        used_score = scores
        proba = None

    # Build rich metrics (Script 2 style) and add split/timestamp
    mb = rich_metrics(y, used_score, y_pred, proba, ece_bins=ece_bins)
    mb["split"] = tag
    mb["timestamp"] = datetime.now(timezone.utc).isoformat()
    save_json(mb, result_dir / f"metrics_{tag}.json")

    # Keep classification report and plots as before (filenames unchanged)
    from sklearn.metrics import classification_report as _clf_rep
    (result_dir / f"classification_report_{tag}.txt").write_text(
        _clf_rep(y, y_pred, labels=[0,1], digits=4, zero_division=0), encoding="utf-8"
    )
    plot_confusion(y, y_pred, result_dir / f"confusion_matrix_{tag}.png", f"Confusion Matrix ({tag}, pos=adv)")
    plot_roc(y, used_score, result_dir / f"roc_curve_{tag}.png", f"ROC Curve ({tag}, pos=adv)")
...
def main():
    ...
    try:
        ...
        # keep candidate_summary.json schema the same (auroc/ap/f1) for compatibility
        cand_metrics = {
            "accuracy": float(accuracy_score(y_val, (cand_scores >= (0.5 if _is_probability_vector(cand_scores) else 0.0)).astype(int))),
            "precision": float(precision_score(y_val, (cand_scores >= (0.5 if _is_probability_vector(cand_scores) else 0.0)).astype(int), zero_division=0)),
            "recall": float(recall_score(y_val, (cand_scores >= (0.5 if _is_probability_vector(cand_scores) else 0.0)).astype(int))),
            "f1": float(f1_score(y_val, (cand_scores >= (0.5 if _is_probability_vector(cand_scores) else 0.0)).astype(int))),
            "auroc": float(roc_auc_score(y_val, cand_scores)) if len(np.unique(y_val)) > 1 else float("nan"),
            "avg_precision": _safe_ap(y_val, cand_scores),
        }
    ...
    # Save candidate summary (VAL) — unchanged schema for compatibility
    cand_summary = [
    ...
    write_reports(result_dir, "val", y_val, val_scores, ece_bins=15)

    test_scores = chunked_predict_scores(winner, X_te, CFG["pred_chunk_rows"], f"Predict TEST ({winner_name})")
    mb_test = write_reports(result_dir, "test", y_te, test_scores, ece_bins=15)

    # Per-subgroup on TEST (NAT vs ADV) == simply class split (console only)
    ...
    try:
        if len(np.unique(y_val)) == 2 and _is_probability_vector(val_scores):
            iso = IsotonicRegression(out_of_bounds="clip")
            val_probs_like = val_scores
            ...
            print("[WARN] Skipping calibration: need 2-class VAL with probabilities.")
    except Exception as e:
    ...
    best_info = {
        ...
        "val_metrics": best["val_metrics"],   # keep original keys for backward-compat in this file
        "test_metrics": mb_test,              # rich metrics for TEST (new schema)
        "meta": winner_meta,
        "target": "is_adv (1=adversarial, 0=natural)"
    }
